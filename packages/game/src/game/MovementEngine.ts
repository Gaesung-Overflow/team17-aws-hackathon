import { Position, Direction, MovementHistory } from './types';
import { getValidDirections, getOppositeDirection, isSameDirection, wasRecentPosition } from './utils';
...
export abstract class MovementEngine {
  ...
  protected getDirectionWeights(validDirections: Direction[]): Map<Direction, number> {
    ...
    validDirections.forEach(dir => {
      ...
      // 되돌아가기 회피
      if (this.lastDirection && isSameDirection(dir, getOppositeDirection(this.lastDirection))) {
        weight = 0.1;
      }
  ...
  protected selectWeightedDirection(weights: Map<Direction, number>): Direction {
