...
export class PlayerEngine extends MovementEngine {
  getNextMove(
    ...
    const hasValidMove = Array.from(weights.values()).some(
      (weight) => weight > 0,
    );
    if (!hasValidMove) {
      return { x: 0, y: 0 };
    ...
    // 랜덤 요소 (10% 확률) - 유효한 방향만 선택
    if (Math.random() < 0.1) {
      const validWeightedDirections = validDirections.filter(
        (dir) => (weights.get(dir) || 0) > 0,
      );
      if (validWeightedDirections.length > 0) {
        const randomDir =
          validWeightedDirections[
            Math.floor(Math.random() * validWeightedDirections.length)
          ];
        this.lastDirection = randomDir;
        this.pathContinuity.updateDirection(randomDir);
    ...
    this.pathContinuity.updateDirection(selectedDirection);
    this.updateHistory(currentPos);
